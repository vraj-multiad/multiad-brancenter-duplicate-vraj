...
class ImageUploadUploader < BaseUploader
  version :jpg do
    def full_filename (path = model.image_upload.file.path)
  ...
  version :original do
    def full_filename(path = model.image_uploader.file.path)
      if model.upload_type == 'library_video' && ENV['ZENCODER_TRANSCODE_ORIGINAL']
  ...
  version :thumbnail do
    def full_filename (path = model.image_upload.file.path)
      case model.upload_type
      when 'library_image', 'logo', 'ac_image', 'ac_creator_template'
    ...
    def url
      case self.model.upload_type
      when 'ac_image', 'library_image', 'library_video', 'logo'
        super
      end
    end
  end
  ...
  def video_thumbnail_filename
    File.basename(self.model.read_attribute(:image_upload), File.extname(self.model.read_attribute(:image_upload))) + '_thumbnail_{{number}}'
  end
end
